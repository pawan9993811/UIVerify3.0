...
namespace VisualUIAVerify.Controls
{
    ...
    public partial class AutomationElementTreeControl : UserControl
    {
        ...
        static bool FindControl(List<UIControl> childList, UIControl node, out int index, List<string> nameList)
        {
            bool allPropsMatched = false;
            bool tempFlag = true; bool duplicateDisplayName = false;
            for (int i = 0; i < childList.Count; i++)
            {
                if (childList[i].Name.StartsWith(node.Name))
                {
                    nameList.Add(childList[i].Name);
        ...
        private static void SetSearchProperties(UIControl control, AutomationElement.AutomationElementInformation ElementInfo)
        {
            ...
            {
                control.SearchCriteria.SearchProperties.Add("AutomationID", ElementInfo.AutomationId,"EqualsTo");
            }
            if (!string.IsNullOrEmpty(ElementInfo.ClassName))
            {
                control.SearchCriteria.SearchProperties.Add("ClassName", ElementInfo.ClassName, "EqualsTo");
            }
            if (!string.IsNullOrEmpty(ElementInfo.Name))
            {
                control.SearchCriteria.SearchProperties.Add("Name", ElementInfo.Name, "EqualsTo");
            }
            if (!string.IsNullOrEmpty(ElementInfo.LocalizedControlType))
            {
                control.SearchCriteria.SearchProperties.Add("ControlType", ElementInfo.LocalizedControlType, "EqualsTo");
            }
        }
